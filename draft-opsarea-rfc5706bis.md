...
## Impact on Security Operations {#sec-impact-secops}
   ...
   Indicators of Compromise (IoCs) {{?RFC9424}} are relied upon by security operators to identify and block malicious activity. The deployment of New Protocol or Protocol Extension may change the type, locations or availability of IoCs and Protocol Designers should outline such changes. Security operators discover, assess, share and deploy IoCs into the network. When subsequent activity then correlates to an IoC, this detected activity can be reacted to by security operators to defend and manage the network effectively. Consideration should be given to ensure operators are able to perform all parts of this IoC lifecycle, as any change will reduce the ability of operators to manage their network consistently.    
   
   Similarly, to be most effective, IoCs are required to be shared at scale so consideration should be given to ensure this is possible. 
